...
---
This section covers creating your first poll, the essential first step for new users to schedule events by collecting date and time preferences from participants. Designed for anyone organizing meetings, events, or decisions—whether personal or team-based—polls integrate seamlessly with your account after [Account Creation and Login](account-creation-and-login). Once created, expand options in [Adding Poll Options](adding-poll-options), invite others via [Inviting Participants](inviting-participants), or view votes in [Viewing Results](viewing-results). For team polls, use [Space Dashboard](space-dashboard).
...
graph TB
    ...
    Share --> End["Poll live! View in [Viewing Results](viewing-results)"]
```
...
| **Notifications** | On | On/Off | Emails you on new votes (configurable in [Notifications and Emails](notifications-and-emails)). |
...
> [!WARNING]  
> After creation, the **Poll Link** is permanent—share carefully. Edit polls later via [Managing Polls](managing-polls).
...
- For next steps: add more options in [Adding Poll Options](adding-poll-options), invite via [Inviting Participants](inviting-participants), or track results in [Viewing Results](viewing-results).
- Team use: Create in a [Space Dashboard](space-dashboard) for collaboration.